...
struct HandJoint {
  ...
  float swelling;
  float dilation;
  float elongation;  // bone elongation or shortening
...
public:
  ...
  static const int kElementsPerJoint = 6;
  ...
  float &dilation(int joint);
  float elongation(int joint) const;
  float &elongation(int joint);
...
void write(cv::FileStorage&, string&, const FullHandPose&);
void read(const cv::FileNode&, libhand::FullHandPose&, libhand::FullHandPose);
...
inline float FullHandPose::swelling(int joint) const {
  return data_[JointOffset(joint) + 3];
}

inline float&FullHandPose::swelling(int joint) {
  return data_[JointOffset(joint) + 3];
}

inline float FullHandPose::dilation(int joint) const {
  return data_[JointOffset(joint) + 4];
}

inline float &FullHandPose::dilation(int joint)  {
  return data_[JointOffset(joint) + 4];
}

inline float FullHandPose::elongation(int joint) const {
  return data_[JointOffset(joint) + 5];
}

inline float& FullHandPose::elongation(int joint)  {
  return data_[JointOffset(joint) + 5];
}
