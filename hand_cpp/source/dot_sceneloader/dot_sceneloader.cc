// -*- mode: c++; c-file-style: "linux"; tab-width: 4; c-basic-offset:4; indent-tabs-mode:nil -*-
...
void DotSceneLoader::processCamera(TiXmlElement *XMLNode, SceneNode *pParent)
{
    ...
    if(pElement)
    {
        Real nearDist = getAttribReal(pElement, "near", -1);
        Real farDist =  getAttribReal(pElement, "far", -1);

        if (nearDist < 0) {
            nearDist = getAttribReal(pElement, "nearPlaneDist", -1);
        }
        if (farDist < 0) {
            farDist = getAttribReal(pElement, "farPlaneDist", -1);
        }
